...
// Conditional Google OAuth import - dynamic
let useGoogleLoginHook = null
import './App.css'
...
function LandingPage({ onNavigate }) {
  ...
  const [googleLoginFn, setGoogleLoginFn] = useState(null)
  
  useEffect(() => {
    // Dynamic import Google OAuth
    import('@react-oauth/google')
      .then((module) => {
        const { useGoogleLogin } = module
        // Hook ni component ichida ishlatish kerak, shuning uchun wrapper funksiya yaratamiz
        const createLoginFn = () => {
          // Bu funksiya component render qilinganda chaqiriladi
      ...
      try {
        ...
        if (!userInfo.email || !userInfo.email.endsWith('@gmail.com')) {
          alert('Iltimos, faqat Gmail akkauntidan foydalaning.')
          return
        }

        // Mijoz sifatida saqlash
        try {
          await customersAPI.create({
            name: userInfo.name || userInfo.given_name || 'Foydalanuvchi',
            email: userInfo.email,
            phone: '', // Telefon keyinchalik to'ldiriladi
            gender: '',
            age: '',
            orders: 0,
            profile: ''
        ...
        alert('Google ma\'lumotlarini olishda xatolik yuz berdi.')
      }
    },
    onError: (error) => {
    ...
    flow: 'auth-code', // Authorization code flow
      ...
      .catch((error) => {
        ...
        setGoogleLoginFn(() => () => {
          alert('Google OAuth paketi o\'rnatilmagan. Iltimos, npm install @react-oauth/google ni bajaring.')
        })
      })
  }, [])

  const googleLogin = googleLoginFn || (() => {
    alert('Google OAuth yuklanmoqda...')
  })
              ...
              <button
                onClick={() => {
                  if (googleLoginFn) {
                    ...
                    alert('Google OAuth hali yuklanmagan. Iltimos, biroz kutib turing.')
                  }
                }}
                disabled={!googleLoginFn}
